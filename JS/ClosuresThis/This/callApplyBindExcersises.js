...
/* 
// Write a function called sumEvenArguments which takes all of the arguments passed to a 
function and returns the sum of the even ones.
...
console.log(sumEvenArguments(1, 3, 2, 6));
...
to bind with the correct value of the keyword this. HINT - if you pass more than two parameters to bind, 
...
function bind(fn, thisArg, ...rest) {
  // if not using ...rest argument
  // var remainingArgs = [].slice.call(arguments,2);
  return function() {
    //without ...rest
    // var innerArgs = [].slice.call(arguments);
    // var allArgs = remainingArgs.concat(innerArgs);
    [].unshift.call(arguments, ...rest);
    return fn.apply(thisArg, arguments);
  };
}
function addFourNumbers(a, b, c, d) {
...
to flip with the correct value of the keyword this and all of the arguments passed to the function 
REVERSED. HINT - if you pass more than two parameters to flip, 
those parameters should be included as parameters to the inner function when it is invoked.
You will have to make use of closure! 

Flip should return a new function that when invoked takes the correct number of required arguments 
to that function which are then reversed. HINT - you will need to use the .length 
